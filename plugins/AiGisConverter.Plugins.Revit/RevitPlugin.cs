...
/// <summary>
/// Contributes the Revit Reader to the host.
/// </summary>
/// <remarks>
/// Registration always succeeds, even when Revit is absent. Whether Revit is actually
/// reachable is a question for <see cref="CheckHealthAsync"/> and for the moment a read is
/// attempted &#8212; not for load time, because Revit may be started after the converter.
/// </remarks>
public sealed class RevitPlugin : PluginBase, IPluginHealthCheck
{
    private const string HostName = "Revit";
    private const int ConnectTimeoutMilliseconds = 2000;

    private IBridgeClient? _bridgeClient;

    /// <inheritdoc />
    public override string Id => "aigis.reader.revit";
        ...
        CancellationToken cancellationToken)
    {
        ...
        registration.AddCapability<IDataSourceReader>(new RevitReader(_bridgeClient, context.Logger));
        registration.AddCapability<ISemanticProvider>(new RevitSemanticProvider());
        registration.AddCapability<IPluginCapabilities>(new RevitCapabilities());
